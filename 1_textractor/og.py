...
class OutputGenerator:
    def __init__(self, response, fileName, forms, tables, folder):
    ...
    def _outputWords(self, page, p):
        ...
        folder = f'{self.folder}/{self.fileName.split(".")[0]}_folder/'
        if not os.path.exists(folder):
            os.makedirs(folder)
        FileHelper.writeCSV("{}{}-page-{}-words.csv".format(folder, self.fileName, p),
    ...
    def _outputText(self, page, p):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        FileHelper.writeToFile("{}{}-page-{}-text.txt".format(folder,self.fileName, p),
                               text)

        textInReadingOrder = page.getTextInReadingOrder()
        FileHelper.writeToFile(
            "{}{}-page-{}-text-inreadingorder.txt".format(folder, self.fileName, p),
            textInReadingOrder)
    ...
    def _outputForm(self, page, p):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        FileHelper.writeCSV("{}{}-page-{}-forms.csv".format(folder, self.fileName, p),
                            csvFieldNames, csvData)
    ...
    def _outputTable(self, page, p):
        ...
        FileHelper.writeCSVRaw(
            "{}{}-page-{}-tables.csv".format(folder, self.fileName, p), csvData)
    ...
    def _outputTablePretty(self, page, p, table_format='github'):
        for table_number, table in enumerate(page.tables):
            ...
            if not os.path.exists(folder):
                os.makedirs(folder)
            FileHelper.writeToFile(
                "{}{}-page-{}-table-{}-tables-pretty.txt".format(folder,
